...
function resizeAndPlay2(element) {
    console.log("resizeAndPlay called for: ", element.id);  // Debugging log

    var cv = document.getElementById(element.id + "Merge");
    if (!cv) {
        console.error("Canvas not found for video: " + element.id);
        return;
    }

    console.log("Video width:", element.videoWidth, "Video height:", element.videoHeight);  // Debugging log
